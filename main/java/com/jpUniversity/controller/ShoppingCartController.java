...
public class ShoppingCartController {
    ...
    public String shoppingCart(Model model, Principal principal) {
        ...
        /*first update shopping cart in case there's price, number of items change or unavailable products*/
        List<CartItem> cartItemList = cartItemService.findByShoppingCart(shoppingCart);

        shoppingCartService.updateShoppingCart(shoppingCart); /*then update the shopping cart*/
    ...
    public String addItem(@ModelAttribute("book")Book book,
        ...
        /*convert from string to integer to check if the requested # is greater than what we have*/
        if (Integer.parseInt(qty) > book.getInStockNumber()) {
            model.addAttribute("notEnoughStock", true);
            return "forward:/bookDetail?id=" + book.getId(); /*forward goes directly to the link keeping the content and faster than redirect*/
        }
